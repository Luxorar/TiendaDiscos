...
export default function DiscoDetail() {
  ...
  const stockLabel = (() => {
    if (loadingStock) return 'Cargando stock...'
    if (stock === null || stock === undefined) return 'Stock no disponible'
    if (stock === 0) return 'Sin stock disponible'
    if (stock <= 5) return `Quedan solo ${stock} unidades`
    return `${stock} unidades disponibles`
  })()

  const stockClass = (() => {
    if (loadingStock || stock === null) return 'detail-stock'
    if (stock === 0) return 'detail-stock out-of-stock'
    if (stock <= 5) return 'detail-stock low-stock'
    return 'detail-stock in-stock'
  })()
      ...
      <div className="disco-detail-content">
        <div className="disco-detail-main">
          ...
          <div className="disco-detail-info">
            ...
            >
              {added ? 'Agregado!' : stock === 0 && !loadingStock ? 'Sin stock' : 'Agregar al Carro'}
            </button>
          </div>
        </div>
